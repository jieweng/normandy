...
package com.pearl.normandy.component.ibizTree
{

import com.pearl.normandy.component.ibizTree.event.MyTreeEvent;
import com.pearl.normandy.component.ibizTree.event.MyTreeListData;
...
[Event(name="addItem", type="com.pearl.normandy.component.ibizTree.event.MyTreeEvent")]
[Event(name="deleteItem", type="com.pearl.normandy.component.ibizTree.event.MyTreeEvent")]
//--------------------------------------
//  Styles
//--------------------------------------

include "PaddingStyles.as";
...
public class Tree extends List implements IIMESupport
{
	...
	private var oldParent:ProjectUserVO;
    ...
    public function Tree()
    {
        ...
        projectUserRo = new RemoteObject();
        projectUserRo.destination = "ro.projectuser";
       	projectUserRo.addEventListener(ResultEvent.RESULT, ResultHandler);
		projectUserRo.addEventListener(FaultEvent.FAULT, ActivityFaultHandler);
    }
	...
	private function ResultHandler(evt:ResultEvent):void{
		...
		//是一个 存储 或更新操作
		if(evt.token.message["operation"] == "saveOrUpdate"){
			...
			if(dragInitiator == this){
	            ...
	            index = getChildIndexInParent(parent, item);
	
				removeChildItem(parent, item, index);
				if(parent && (parent as ProjectUserVO).children.length == 0){
					
					(parent as ProjectUserVO).children = null;
				} 
				if(_dropData.parent){
	        
		        	item.supervisorId = _dropData.parent.userId;
		        }
			}
			if(item.id == 0){
				item.id = (evt.result as ProjectUserVO).id;
			}
			addChildItem(_dropData.parent, item, _dropData.index);
			expandChildrenOf(_dropData.parent, true);
		
		//是一个删除操作
		}else if(evt.token.message["operation"] == "deleteProjectUser"){
			
			var selItemParent:ProjectUserVO = getParentItem(selectedItem);
			if(selItemParent != null){
					
				var index2:uint = selItemParent.children.getItemIndex(selectedItem);
				removeChildItem(selItemParent, selectedItem, index2);
				if(selItemParent.children == null){
					return;
					
				}else if(selItemParent.children.length == 0){
				
					selItemParent.children = null;
				}
			}
		}
        ...
        event.items = [(_rootModel as ArrayCollection).getItemAt(0)];
        collection.dispatchEvent(event);
	}
	
	//请求后台后失败回调函数
	private function ActivityFaultHandler(event:FaultEvent):void{
		
 		if(event.token.message["operation"] == "deleteProjectUserTO"){
		
		//是一个存储或更新操作
		}else if(event.token.message["operation"] == "saveOrUpdate"){
			
			if(dragInitiator == this && item.id > 0){
				
				if(oldParent.projectRoleId == Constants.PROJECT_ROLE_AD_NUM){
					item.projectRoleId = Constants.PROJECT_ROLE_LEAD_NUM;
					item.projectRoleName = Constants.PROJECT_ROLE_LEAD;
				}else if(oldParent.projectRoleId == Constants.PROJECT_ROLE_LEAD_NUM){
					item.projectRoleId = Constants.PROJECT_ROLE_ARTIST_NUM;
					item.projectRoleName = Constants.PROJECT_ROLE_ARTIST;
				}
			}
		} 		
		//弹出错误信息
     ...
     *  @private
     */
     private var enableDrop:Boolean = false;
     private var dropParent:Object;
     private var item:ProjectUserVO;
	...
	/*  private function expandItemFun(data:ProjectUserVO):void{

		for(var i:uint; i < data.children.length; i++){
			expandedItem(data, true, true, true);
			var p:ProjectUserVO;
			...
			for(var i:uint; i < arr.length; i++){
				...
				expandItemFun(arr.getItemAt(i) as ProjectUserVO);
			}
		}
	} */
	...
	public function set isRemove(val:Boolean ):void{
		...
		confirm(resourceManager.getString("Language","alert.delete_projectuser"),
			   				resourceManager.getString("Language","title.confirmation"), confirmRemoveProjectUserFun);
	}
	
	//做release 操作
	private var _isRelease:String;
	public function set isRelease(val:String):void{
		
		_isRelease = val;
		var index:int = getItemIndex(selectedItem) + 1;
		var renderer:IListItemRenderer = listItems[index - 1][0];
        if (renderer is IDropInListItemRenderer)
        {
            var di:IDropInListItemRenderer = IDropInListItemRenderer(renderer);
            var treeListData:MyTreeListData = MyTreeListData(di.listData);
            treeListData = MyTreeListData(makeListData(treeListData.item,
                treeListData.uid, treeListData.rowIndex));
			if(_isRelease == "OFF" && (treeListData.item as ProjectUserVO).status == Constants.RESOURCE_PROJECT_STATUS_ASSIGNED){
				(treeListData.item as ProjectUserVO).status = Constants.RESOURCE_PROJECT_STATUS_RELEASED;
			}else if(_isRelease == "ON" && (treeListData.item as ProjectUserVO).status == Constants.RESOURCE_PROJECT_STATUS_RELEASED){
				(treeListData.item as ProjectUserVO).status = Constants.RESOURCE_PROJECT_STATUS_ASSIGNED;
			}	                
            di.listData = treeListData;
            renderer.data = renderer.data;  // this forces eval of new listData
            (renderer as MyTreeItemRenderer).invalidateProperties();
            projectUserRo.updateProjectUserStatus(treeListData.item);
        }	
	}
	...
	private function confirmRemoveProjectUserFun(evt:CloseEvent):void{
		if (evt.detail == Alert.YES) {
			var selItemParent:ProjectUserVO = getParentItem(selectedItem);			
			projectUserRo.deleteProjectUser(selectedItem);
			if(selItemParent.children == null){
				return;
	...
	private function confirm(text:String,  title:String, confirmHandler:Function):void {
		// instantiate the Alert box
		var a:Alert = Alert.show(text, title, Alert.YES|Alert.NO, 
			this, confirmHandler, null, Alert.NO);
    ...
    */
    ...
    //设置选择对象
    private var _selectObj:ProjectUserVO;
    public function set selectObj(val:ProjectUserVO):void{
    
    	_selectObj = val;
    	
    }
    
    public function get selectObj():ProjectUserVO{
    ...
    {
    	if(isInit && dataProvider is ArrayCollection && (dataProvider as ArrayCollection).length > 0){
        	expandAll(dataProvider[0], true, isInit);
        	isInit = false;
        }  
       ...
       /*  if(_rootModel.length != 0){
        	
        	var data:ProjectUserVO = (_rootModel as ArrayCollection).getItemAt(0) as ProjectUserVO;
			expandedItem(data, true, true, true);      
        } */
       /*  if(_rootModel.length != 0){
    ...
    override public function showDropFeedback(event:DragEvent):void
    {
        ...
        var offset:int = 0;
        updateDropData2(event);
        if(_dropData.rowIndex > collection.length){
    	...
    	if (event.dragSource.hasFormat("treeItems")){
	    	var items:Array = event.dragSource.dataForFormat("treeItems") as Array;
	 		item = items[0] as ProjectUserVO;
			if (_dropData.parent)
	        {
 	        	if(checkParentSame(_dropData.parent, item)){
	        		hideDropFeedback(event);
	        		return; 
	        	} 
	/*             offset = getItemIndex(iterator.current);
	            depth = getItemDepth(_dropData.parent, Math.abs(offset - getItemIndex(_dropData.parent))); */
	            var projectUser:ProjectUserVO = _dropData.parent as ProjectUserVO;
	        	if(projectUser.projectRoleId == 4){
	        		hideDropFeedback(event);
					return;
	        	}else if(projectUser.projectRoleId == 3){
	        		depth = 1;
	        	}else if(projectUser.projectRoleId == 2){
	        		depth = 0;
	        	}
    ...
    {
        var treeListData:MyTreeListData = new MyTreeListData(itemToLabel(data), uid, this, rowNum);
    ...
    */
	...
	//扩展所有节点
	public function expandAll(item:Object, isOpen:Boolean, isInit:Boolean):void{
		
        if (iterator == null)
            return;
        
        if (isInit && isBranch(item))
        {
            dispatchTreeEvent(TreeEvent.ITEM_OPENING,item, null, null, isOpen, false, true);
            ...
            if (childItems)
            {
    			...
    			while (!cursor.afterLast)
    			{
                    if (isBranch(cursor.current))
                        expandAll(cursor.current, isOpen, isInit);
    				cursor.moveNext();
    			}
            }
        }
    ...
    private function getIndent():Number
    {
        ...
        for (var p:String in _openItems)
        {
            ...
            depth = Math.max(getItemParentStack(_openItems[p]).length + 1, depth);
        }
    ...
    */
     ...
     */
    private function getItemParentStack(item:Object):Array
    {
        var stack:Array = [];
        if (item == null)
            return stack;
	...
	{	
		...
		for (var i:int = 0; i < selectedItems.length; i++)
		{
			...
			var stack:Array = getItemParentStack(item);
     ...
     */
    protected function initListData(item:Object, treeListData:MyTreeListData):void
    {
        if (item == null)
            return;
...
/*          if (!tween){
    ...
    override protected function mouseDownHandler(event:MouseEvent):void
    {
        ...
        selectObj = (event.currentTarget as Tree).selectedItem as ProjectUserVO;
    }
	...
	override protected function dragStartHandler(event:DragEvent):void{
		...
		if(event.dragInitiator == this){
			...
			if((this.selectedItem as ProjectUserVO).projectRoleId == Constants.PROJECT_ROLE_AD_NUM ||
				(this.selectedItem as ProjectUserVO).projectRoleId == Constants.PROJECT_ROLE_LEAD_NUM){
    ...
    override protected function dragDropHandler(event:DragEvent):void
    {
		...
		if (event.dragSource.hasFormat("treeItems"))
		{
        	var items:Array = event.dragSource.dataForFormat("treeItems") as Array;
 			...
 			if(item.supervisorId == Constants.PROJECT_ROLE_AD_NUM){
 				return;
 			}
        	var i:int;
        	var n:int;
        	...
        	oldParent = getParentItem(item) as ProjectUserVO;
        	// If we're dropping onto ourselves or a child of a descendant then dont actually drop
			updateDropData2(event);	
			if(_dropData.parent != null && _dropData.parent.children != null){
			...
			var dropParentStack:Array = getItemParentStack(_dropData.parent);
			dropParentStack.unshift(_dropData.parent);
			if(dropParentStack.length == 1 && dropParentStack[0] == null){
				return;
			}
			if(dropParentStack.length > 2){
				
				return;
			} 
			if(item.projectRoleId == Constants.PROJECT_ROLE_LEAD_NUM && dropParentStack.length == 2){
			...
			if(dropParentStack.length == 1 && dropParentStack[0] != null){
				...
				item.projectRoleName = Constants.PROJECT_ROLE_LEAD;
				item.projectRoleId = Constants.PROJECT_ROLE_LEAD_NUM;
			}else if(dropParentStack.length == 2){
...
//				dragInitiator = event.dragInitiator;
 ...
 //       		dragInitiator = event.dragInitiator;
			...
			if(item.status == null){
				item.status = Constants.RESOURCE_PROJECT_STATUS_ASSIGNED;
			}
	...
	//核查父亲节点是否是同一节点
 	private function checkParentSame(parent:ProjectUserVO, item:ProjectUserVO):Boolean{
		...
		var child:ProjectUserVO;
		for(var i:uint = 0; i < len; i++){
		
			child = parent.children.getItemAt(i) as ProjectUserVO;
     ...
     *  @private
     */
    private function updateDropData1(event:DragEvent):void
    {
        var rowCount:int = rowInfo.length;
        var rowNum:int = 0;
        var yy:int = rowInfo[rowNum].height;
     ...
     *  @private
     */
    private function updateDropData2(event:DragEvent):void
    {
    	dropParent = getParentItem(selectedItem);
        var rowCount:int = rowInfo.length;
        var rowNum:int = 0;
        var yy:int = rowInfo[rowNum].height;
        var pt:Point = globalToLocal(new Point(event.stageX, event.stageY));
